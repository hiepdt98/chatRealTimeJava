var CreateProxy = function(wsUri) {
	...
	var displayMessageMulti = function(msgs) {
		...
		for(var i = 0;i<msgs.length;i++){
			...
			var textnode = document.createTextNode(msgs[i]);
	...
	var sendMessageGetMessage = function(nameRoom) {
		if (websocket != null && websocket.readyState == 1) {		
			var message = { messageType: 'GETMESSAGE', message: nameRoom };
...
//		  }
		for(var i = 0;i<room.length;i++){
			...
			button.onclick = function(e){
				console.log(e);
				document.getElementById('nameRoom').innerHTML = e.target.innerHTML;
				sendMessageGetMessage(e.target.innerHTML);
			}
	...
	return {
		login: function() {
			...
			if (websocket == null) {
		        ...
		        websocket.onmessage = function(e) {
		        	...
		        	if(obj.MessageType == "SHOWMESSAGE"){
		        		console.log(obj);
		        		message = JSON.parse(obj.Message);
			        	displayMessageMulti(message.arrRoom);
		        	}
		...
		sendMessage: function() {
			...
			if (websocket != null && websocket.readyState == 1) {
				...
				var room  = document.getElementById('nameRoom').innerHTML;
				console.log(input, console.log(room))
				var message = { messageType: 'MESSAGE', message: input, nameRoom: room};
