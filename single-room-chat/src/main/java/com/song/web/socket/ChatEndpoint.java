...
import com.song.chat.connect.DAO;
...
//			rooms.add(room);
		...
		if(chatMessage.getMessageType() == MessageType.GETMESSAGE) {
			System.out.println("vao day lay message of room");
			String nameRoom = chatMessage.getMessage();
			ArrayList<Message> messages = new ArrayList<Message>();
			try {
				messages = new DAO().getMessage(nameRoom);
			} catch (Exception e) {
				e.printStackTrace();
			}
			MessageSend messageSendMessage = new MessageSend();
			
			messageSendMessage.setMesage("SHOWMESSAGE");
			for(Message mes : messages) {
				System.out.println(mes.getContent());
				messageSendMessage.addRoom(mes.getContent());
			}
			Gson gson = new Gson();
			//convert onject to json to send client
			String json = gson.toJson(messageSendMessage);
//			// add name message and rooms into message to client
			ResponseMessage responsegetMessage = new ResponseMessage("SHOWMESSAGE",json);
			String jsonGetMess = gson.toJson(responsegetMessage);
			session.getBasicRemote().sendText(jsonGetMess);
		...
		if(chatMessage.getMessageType() == MessageType.MESSAGE) {
			...
			String message =nameClient + ":" +chatMessage.getMessage();
			
			String nameRoom =(String) chatMessage.getNameRoom();
			// save mess to db
			Message messagedb = new Message(message, java.time.LocalDateTime.now().toString(), nameRoom);
			try {
				new DAO().addMessage(messagedb);
			} catch (Exception e) {
				e.printStackTrace();
			}
